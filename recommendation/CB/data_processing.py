...
class DataProcessing:
    ...
    def process_user_data(self, file='../../data/ml-1m/users.dat'):
        fp = pd.read_table(file, sep='::', engine='python', names=['UserID', 'Gender', 'Age', 'Occupation', 'Zip_code'])
    ...
    def process_rating_data(self, file='../../data/ml-1m/ratings.dat'):
        fp = pd.read_table(file, sep='::', engine='python', names=['UserID', 'MovieID', 'Ratings', 'TimeStamp'])
    ...
    def prepare_item_profile(self, file='../../data/ml-1m/use/movies.csv'):
        ...
        for item in item_ids:
            ...
            # 保存dict时，key只能为str，所以要对itemID进行str()转化
        self.item_matrix = {}
        ...
        print("item信息计算完成，路径为{}".format('data/ml-1m/use/item_profile.json'))
    ...
    def prepare_user_profile(self, file='../../data/ml-1m/use/ratings.csv'):
        ...
        print("user信息计算完成，路径为{}".format('data/ml-1m/use/user_profile.json'))
